...
On a busy node, a payment API and a batch job can look the same to Linux even when Kubernetes knows they are not.
...
Unlabeled tasks (sshd, kubelet, the load generator) stay **default**. If unknown work goes to background, you can stall the machine.
...
**Kernel person:** Three FIFO DSQs, no LOCAL fast-path for background, no preempt-on-enqueue. Tick yields if the latency DSQ is non-empty. Here is the harness. Review welcome.
